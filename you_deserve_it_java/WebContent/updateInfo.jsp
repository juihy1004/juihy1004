        ...
        <div class="container">
          <div class="row">         
            <div class="col-md-12">
              ...
              <br><br>
                <h2 class="product-title">회원 정보 수정</h2>
                <ol class="breadcrumb">
                  <li><a href="index.jsp"><i class="ti-home"></i> 홈 </a></li>
                  <li class="current">회원 정보 수정</li>
                </ol>
              </div>
            </div>
          </div>
          ...
          <!-- 왼쪽 메뉴바 -->
              ...
              <div class="right-sideabr">
                <div class="inner-box">
                  ...
                  <ul class="lest item">
                     ...
                     <li><a class="active" href="updateInfo.jsp">- 내 정보 수정 </a></li>  
            ...
            <div class="col-md-8 col-sm-8 col-xs-12">
              <div class="job-alerts-item candidates">
                <h3 class="alerts-title" style = "text-align:center">회원 정보 수정</h3>
                    ...
                    <input type = "hidden" name = "command" value = "updateinfo">
                      ...
                      <div class="form-group">
                        <div class="input-icon">
                           <i class="ti-lock"></i>
                          <input type="password" id="sender-email" class="form-control" name="pwd" required="required" placeholder="변경할 비밀번호를 입력하세요">
                        </div>
                      </div>
                      ...
                      <div class="form-group">
                        <div class="input-icon">
                          <i class="ti-lock"></i>
                          <input type="password" id="sender-email" class="form-control" name="pwdChk" required="required" placeholder="변경할 비밀번호 확인">
                          <span style="display:none; color:red;" id="pwChkSpan">패스워드가 일치하지 않습니다. 다시 입력하세요.</span>
                        </div>
                      </div>
                      ...
                      <div class="form-group">
                        <div class="input-icon">
                         <i class="ti-user"></i>
                          <input type="text" id="sender-email" class="form-control" name="nick" required="required" value="<%=m.getNickname() %>" placeholder="변경할 닉네임을 입력하세요">
                          <span style="display:none; color:red;" id="nickChkSpan">이미 존재하는 닉네임입니다. 다시 입력하세요.</span>
                        </div>
                      </div>
                      ...
                      <div class="form-group">
                        <div class="input-icon">
                          <i class="ti-email"></i>
                          <input type="email" class="form-control" name = "email" required="required" value="<%=m.getEmail() %>" placeholder="변경할 이메일을 입력하세요">
                          <span style="display:none; color:red;" id="emailChkSpan">이미 등록된 이메일입니다. 다시 입력하세요.</span>
                          <span style="display:none; color:red;" id="emailChkSpan2">유효하지 않는 이메일 주소입니다. 다시 입력하세요.</span>
                        </div>
                      </div> 
                     
                      <button class="btn btn-common log-btn" id="updateBtn">회원 정보 수정</button> 
                    </form>
                  </div>
                </div>
      ...
      $("input[name='nick']").change(function() { 
         ...
         $.ajax({ 
            ...
            data: {
               ...
               url : "controller.do?command=updateNickChk",
               dataType : "json",
               success : function(data) { 
                  if(inputed==ssnick) {
                     $("#nickChkSpan").css("display", "none");
                 nres = true;
                 res();
                  } else if(!data.res) { 
                $("#nickChkSpan").css("display", "");
                nres = false;
                res();
                  } else { 
                 $("#nickChkSpan").css("display", "none");
         ...
         var ssemail = '${vo.email}';
            ...
            $.ajax({                  
               data: {
                  ...
                  url : "controller.do?command=updateEmailChk",
                  dataType : "json",
                  success : function(data) { 
                if (inputed==ssemail) {   //입력한 이메일 주소와 세션에 저장된 멤버객체의 이메일 주소가 같으면
                        $("#emailChkSpan").css("display", "none");
                        $("#emailChkSpan2").css("display", "none");
                        eres = true;
                        res();
                     } else if (!chkEmail(inputed)) {   //입력한 이메일 주소의 유효성 검사
                        $("#emailChkSpan").css("display", "none");
                        $("#emailChkSpan2").css("display", "");
                        eres = false;
                        res();
                     } else if (!data.res) {    //입력한 이메일 주소가 이미 사용중일 경우
                        $("#emailChkSpan").css("display", "");
                        $("#emailChkSpan2").css("display", "none");
                        eres = false;
                        res();
                     } else {
        ...
        $("input[name='pwdChk']").change(function(){
           pwChk();
        });
        function pwChk() {
            if($("input[name='pwd']").val() != $("input[name='pwdChk']").val()){
               if($("input[name='pwdChk']").val()!=''){
         ...
         function res(){
            if($("input[name='pwdChk']").val() != "" && nres == true && eres == true && pres == true){
